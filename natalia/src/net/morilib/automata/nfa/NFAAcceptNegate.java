 ...
 */
public final class NFAAcceptNegate<T, A, B> extends NFAWrapper<T, A, B> {

	//
	private A acc;

	//
	private NFAAcceptNegate(NFAObject<T, A, B> wrapee, A acc) {
		this.wrapee = wrapee;
		this.acc = acc;
	}
	 ...
	 * @param nullable
	 */
	public static<T, A, B> NFAAcceptNegate<T, A, B> newInstance(
			NFAObject<T, A, B> wrapee, A acc) {
		return new NFAAcceptNegate<T, A, B>(wrapee, acc);
	}

	/* (non-Javadoc)
	 * @see net.morilib.automata.nfa.NFAObject#clone()
	 */
	protected NFAAcceptNegate<T, A, B> clone() {
